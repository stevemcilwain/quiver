...
qq-recon-networks-amass-asns() {
  ...
  print -z "amass intel -org ${__ORG} | cut -d, -f1 >> ${__PROJECT}/networks/asns.txt "
}
...
qq-recon-networks-bgpview-ipv4() {
  ...
  __check-asn
  mkdir ${__PROJECT}/networks
  print -z "curl -s https://api.bgpview.io/asn/${__ASN}/prefixes | jq -r '.data | .ipv4_prefixes | .[].prefix' >> ${__PROJECT}/networks/ipv4.txt"
}
...
qq-recon-networks-bgpview-ipv6() {
  ...
  __check-asn
  mkdir ${__PROJECT}/networks
  print -z "curl -s https://api.bgpview.io/asn/${__ASN}/prefixes | jq -r '.data | .ipv6_prefixes | .[].prefix'  >> ${__PROJECT}/networks/ipv6.txt"
}
